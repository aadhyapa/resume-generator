...
def matchmaker(job_desc_embedding, resume_bullet_embeddings):
    """
    Ranks the resume bullets based on the cosine similarity between the job description embedding and the resume bullet embeddings.
    ...
    # sort by score
    resume_bullet_embeddings.sort(key=lambda x: x['score'], reverse=True)
